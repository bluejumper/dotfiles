clear
echo
echo "Welcome back."
echo
cal
echo
fortune
echo
echo "	\"With the new day comes new strength and new thoughts.\" - Eleanor Roosevelt"
echo
echo "	\"Approach the game with no preset agendas and you'll probably come away surprised at your overall efforts.\" - Phil Jackson"
echo
echo "	\"Let go of the thoughts that don't make you strong.\" Unknown [Momentum 20/3/18]"
echo
echo "	\"What would you do if you weren't afraid? I would say automate your day job, so that way you can focus on the impossible!\" - David Swafford, Facebook Datacentre Engineer"
echo
echo "	\"Wrinkles should merely indicate where smiles have been.\" - Mark Twain"
echo
echo "	\"Nothing ever becomes real till it is experienced.\" - John Keats"
echo
echo "	\"Nothing is so fatiguing as the eternal hanging on of an uncompleted task.\" - William James"
echo
echo "	\"It takes as much energy to wish as it does to plan.\" - Eleanor Roosevelt"
echo
echo "	\"Goal? Next 15 minutes.\" - Benayak Karki"
